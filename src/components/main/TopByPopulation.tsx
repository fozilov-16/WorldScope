...
import { getCountriesTopPopulation } from "@/src/reducers/api";
import { useAppDispatch, useAppSelector } from "@/src/store/hooks";
import { ArrowRight, Heart } from "lucide-react";
import { useLocale, useTranslations } from "next-intl";
import Link from "next/link";

export default function TopByPopulation() {
  const dispatch = useAppDispatch();
  const locale = useLocale();
  const countries = useAppSelector((state) => state.todos.topPopulationData);
  const t = useTranslations("TopByPopulation");

  useEffect(() => {
    dispatch(getCountriesTopPopulation(locale));
  }, [dispatch]);
